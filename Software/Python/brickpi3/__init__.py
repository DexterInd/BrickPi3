...
# Global SPI object (initialized when first BrickPi3 instance is created)
BP_SPI = None
...
class BrickPi3(object):
    ...
    def __init__(self, addr = 1, detect = True): # Configure for the BrickPi. Optionally set the address (default to 1). Optionally disable detection (default to detect).
        ...
        # Initialize SPI on first BrickPi3 instantiation
        if BP_SPI is None:
    ...
    def spi_transfer_array(self, data_out):
        ...
        """
        print(f"[DEBUG] SPI OUT: {[hex(b) for b in data_out]}")
        reply = BP_SPI.xfer2(data_out)
        print(f"[DEBUG] SPI IN:  {[hex(b) for b in reply]}")
        return reply
    ...
    def set_sensor_type(self, port, type, params = 0):
        ...
        """
        print(f"[DEBUG] set_sensor_type: port={port}, type={type}, params={params}")
        for p in range(4):
            if port & (1 << p):
        ...
        print(f"[DEBUG] set_sensor_type outArray: {[hex(b) for b in outArray]}")
        self.spi_transfer_array(outArray)
    ...
    def get_sensor(self, port):
        ...
        if port == self.PORT_1:
            ...
            return
        print(f"[DEBUG] get_sensor: port={port}, type={self.SensorType[port_index]}, message_type={message_type}")
